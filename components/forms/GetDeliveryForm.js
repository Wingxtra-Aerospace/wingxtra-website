...
export default function GetDeliveryForm() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      const response = await fetch('/api/get-delivery', {
        method: 'POST',
  ...
  return (
    <form onSubmit={handleSubmit} className="form-grid">
      ...
      <label>
        Phone
        <input required value={form.phone} onChange={(e) => setForm((prev) => ({ ...prev, phone: e.target.value }))} />
      </label>
      <label>
        Email
